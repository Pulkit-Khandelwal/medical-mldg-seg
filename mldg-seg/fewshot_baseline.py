...
class ModelBaseline:
    ...
    def setup_path(self):
        ...
        self.train_paths = ['/path/to/images/june25/verse_few_fov.txt']
        self.val_paths = ['/path/to/images/june25/verse_val.txt']
    ...
    def configure(self):
        PATH = '/path/to/images/code_unet/saved_models/meta_learning.tar'
        tmp=torch.load(PATH)
    ...
    def test(self, ite):
        ...
        for im in range(len(d1)):
            ...
            for batch in range(1, total_batches_per_image + 1):
                ...
                del outputs, val_loss, images_test, labels_test
                torch.cuda.empty_cache()
        ...
        if mean_acc > self.best_accuracy_val:
            ...
            outfile = os.path.join('/path/to/images/code_unet/saved_models/', 'fewshot_meta_learning_verse_june25_images_fov_7.tar')
    ...
    def unseen_fourth_mod(self):
        ...
        PATH = '/path/to/images/code_unet/saved_models/baseline_spine_th_middle.tar' # unseen fourth modality
        #meta_learning_spine_xvert_second_run
        tmp=torch.load(PATH)
        pretrained_dict=tmp['state']
        print(">>>> let us see the state pretrained_dict", tmp['ite'])
        model_dict=self.network.state_dict()
        ...
        for im in range(num_images):
            ...
            for batch in range(1, total_batches_per_image + 1):
                ...
                if np.sum(l_test) != 0:
                    ...
                    file_name_image = '/path/to/images/code_unet/saved_images_base_th_middle/' + 'image_' + str(im+1)+ '_batch_' + str(x) + '_.nii.gz'
                    save_nifti(np.reshape(images_test.cpu().numpy(), (64,64,64)), file_name_image)

                    file_name_label = '/path/to/images/code_unet/saved_images_base_th_middle/' + 'label_' + str(im+1)+ '_batch_' + str(x) + '_.nii.gz'
                    save_nifti(np.reshape(labels_test.cpu().numpy(), (64,64,64)), file_name_label)

                    file_name_pred = '/path/to/images/code_unet/saved_images_base_th_middle/' + 'pred_' + str(im+1)+ '_batch_' + str(x) + '_.nii.gz'
                    save_nifti(np.reshape(predicted_classes.astype(float), (64,64,64)), file_name_pred)
