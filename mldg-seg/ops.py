...
def conv3d(inputs, weight, bias, meta_step_size=0.001, stride=1, padding=1, dilation=1, groups=1, meta_loss=None,
           stop_gradient=False, kernel_size=3):
    inputs = inputs.cuda()
    weight = weight.cuda()
    bias = bias.cuda()
    #print(meta_loss, meta_step_size)

    if meta_loss is not None:
        #print(">>>>> IN OPS <<<<<")

        if not stop_gradient: # this is what actually gets called
            grad_weight = autograd.grad(meta_loss, weight, create_graph=True)[0]
