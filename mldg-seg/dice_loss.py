"""
Changes have been made to this code than the one where it was first adapted from. See references in the README.
"""
import torch
...
class DiceCoeff(nn.Module):
    ...
    def forward(self, input, target):
        self.inter = np.dot(input.flatten(), target.flatten()) + 0.0001
        self.union = np.sum(input) + np.sum(target) + 0.0001
        t = 2 * self.inter / self.union
        return t
...
class GeneralizedDiceLoss(nn.Module):
    ...
    def __init__(self, epsilon=1e-5, weight=None, ignore_index=None, sigmoid_normalization=False, num_classes=2):
    ...
    def forward(self, input, target):
        # get probabilities from logits
        input = self.normalization(input)
        #print(type(target), np.shape(target), len(target))
        ...
        target_one_hot = torch.from_numpy(target_one_hot).float().cuda()

        assert input.size() == target_one_hot.size(), "'input' and 'target' must have the same shape"

        # mask ignore_index if present
        if self.ignore_index is not None:
            mask = target_one_hot.clone().ne_(self.ignore_index)
            mask.requires_grad = False
